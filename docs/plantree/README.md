...
| [agentic-loop-workflow](plans/agentic-loop-workflow/README.md) | In progress | Design and land a state-machine-driven multi-agent workflow loop where `frontdesk` only handles user interaction while planner, broker, orchestrator, execution nodes, monitor, and plan steward roles advance work through scripted state transitions. |
