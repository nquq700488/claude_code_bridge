...
6. Active parent cannot finish until the child result arrives: add
   `--chain`, then stop for continuation.
...
| Active parent needs short child result | `--chain --compact` | Child result resumes parent as a compact continuation |
| Active parent needs full child result | `--chain --artifact-reply` | Most useful callback combination for reports or analysis |
...
| Exact input plus full output | `--artifact-io` | Add `--chain` if an active parent depends on it |
...
- `--chain --silence`: callback needs a result; silence says success should
  not interrupt.
