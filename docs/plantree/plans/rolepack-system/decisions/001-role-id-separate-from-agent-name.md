...
package id such as `ccb.archi` is better for versioning, locks, and community
distribution.
...
Built-in CCB roles use ids such as `ccb.archi`. Community roles use
`<publisher>.<role>`. Project-local agent names may use the short role name by
default, but users may rename the agent when a project already has a conflicting
name.
