...
通信逻辑是 CCB 的核心。用户看到的是 \cmd{ccb ask agent1 ...}、\cmd{ccb watch job_xxx}、\cmd{ccb inbox talk1}、\cmd{ccb ack talk1}，源码里实际流动的是 envelope、job、message、attempt、inbound event、reply、chain edge 和 completion decision。
...
\begin{itemize}
  \item 解析 \cmd{--compact}、\cmd{--silence}、\cmd{--chain}、\cmd{--artifact-request}、\cmd{--artifact-reply}、\cmd{--artifact-io}。
  ...
  \item 保持普通 ask 的原始正文不变；稳定 reply/cancel 规则由 managed
  provider memory 承载。\cmd{--compact} 和 \cmd{--silence} 仅在没有显式
  输出要求时追加一行 \src{CCB\_REPLY\_MODE} 动态标识。
...
CCB 不再把相同的 reply guidance 和 cancel-file 长说明附加到每个 job。
Provider materialization 会把稳定通信规则写入 managed memory；每次请求只保留
用户正文、完成绑定所需的 \src{CCB\_REQ\_ID}，以及确有需要时的一行
\src{CCB\_REPLY\_MODE}。取消以 execution service 的 provider interrupt 为
主路径，cancel flag 仅作为 managed memory 中记录的协作兼容信号。
...
\cmd{--chain} & 把子任务结果作为 continuation 送回当前 agent。适合 parent 需要 child 结果才能完成的任务。 \\
...
\begin{itemize}
  ...
  \item 解析 result chain child continuation。
...
\section{active-job followup}

\cmd{followup <job_id> --message <correction>} 是 dispatcher control operation，
不是普通通信提交。dispatcher 在 chain-transition lock 内重新读取指定 job，要求
它仍是 target slot 的 exact running job，再向 execution adapter 查询显式
capability。能力缺失时必须返回 \src{rejected}，已经 terminal 时返回
\src{too_late}，provider 已观测终态或 cancel/completion 赢得竞态时返回
\src{terminal}；不得改为 pane-wide send-keys、普通 queued job、hidden retry 或
另一 provider。

能力通过后，dispatcher 先追加 \src{accepted} durable outbox 记录，再调用
adapter 注入。daemon restart 只重放仍为 \src{accepted} 的记录，并复用
\src{followup_id} 作为 provider idempotency key。Codex adapter 使用共享 managed
app-server 的 \src{turn/steer}，同时提交 \src{threadId}、
\src{expectedTurnId} 和 \src{clientUserMessageId}；任何 active-turn precondition
失败都由终态赢得。execution poll、followup injection、cancel 和 finish 对 active
submission 的提交转换由同一个 execution lock 串行化；slow poll 在锁外运行，
若 injection 或 terminal transition 已更换 active submission，其迟到结果必须
丢弃。remote TUI 分支还必须写入与 socket path 匹配的 owned marker；只有 socket
而没有 marker 的 local fallback 不具备此 capability。dispatcher terminalization
仍由既有 chain-transition authority 决定。

\section{result chain}

result chain 不是同步等待。chain mode 由 \src{route_options["mode"] == "chain"} 表示，核心实现位于 \src{lib/ccbd/services/dispatcher_runtime/callbacks.py} 和 \src{lib/message_bureau/callback_edges.py}。

active CCB task 内发起普通 nested ask 会被拒绝，除非它使用 \cmd{--chain} 或 \cmd{--silence}。这是因为 parent agent 如果需要 child 结果，应当显式建立 chain edge；如果不需要结果，应当显式选择 fire-and-forget。
...
\begin{itemize}
  ...
  \item parent 不能已有 outstanding result chain。
  \item chain continuation job 不能再 \cmd{--chain} 回该 continuation 的 original caller；它必须直接完成当前 turn，由 CCB 把 completion 继续向上游投递。
\end{itemize}

child 完成后，CCB 不把 child reply 直接投递给 original caller。它记录 child reply，向 parent agent 提交 continuation job，并更新 chain edge。continuation job 的 final answer 才是上游投递路径；agent 不应再用 ask 把 final result 发送给 original caller。repair 和 timeout sweep 可以恢复或失败 pending result chain edges。
...
\begin{enumerate}
  ...
  \item 在 chain transition lock 内重新检查 terminal winner，再写 completion
        snapshot 和 terminal event。
  \item 通过与 normal completion 相同的 callback authority 记录
        message-bureau terminal state。
  \item 解析 reply-delivery terminal state。
\end{enumerate}

取消不是简单从队列删除。它必须把执行层和通信层都推进到一致终态，否则 trace、inbox、queue 和 watch 会互相矛盾。
如果 chain child 被取消，CCB 会把 structured cancelled result 作为唯一的
continuation 送回 parent，由 parent 完成 original task；不会等待 dispatcher
restart 修复 pending edge。普通任务的 empty cancel 记录为从创建起已 consumed
的 \src{completion_notice}，因此 trace 可见但不增加 caller mailbox depth。
有 partial output 或 reply artifact 时仍使用普通 exactly-once reply delivery。
completion 与 cancel 并发时，先写入的 terminal job 胜出，另一方不得覆盖
snapshot、attempt、reply 或 callback edge。
