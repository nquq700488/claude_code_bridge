...
child 完成后，CCB 不把 child reply 直接投递给 original caller。它记录 child reply，向 parent agent 提交 continuation job，并更新 callback edge。repair 和 timeout sweep 可以恢复或失败 pending callback edges。
