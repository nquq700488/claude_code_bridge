...
\begin{itemize}
  ...
  \item \src{docs/dsh-service-provider-contract.md}
\end{itemize}
...
官方 DeepSeek Harness 是一个特殊边界：\src{dsh} 启动 \cmd{dsh web} 服务，request、reply 与 native terminal 都走 loopback HTTP/WebSocket。当前 POSIX launcher 可以把 host wrapper 放在 managed pane 中，但该 pane 只承担 lifecycle/log carrier；CCB 不向它键入 prompt，不从 pane capture 提取 reply，也不以 quiet time 或 process exit 判定完成。完整约束见 \src{docs/dsh-service-provider-contract.md}。把 agent 简单标成 \src{headless} 并不等于已经实现 service supervisor。
...
Codex、Claude、Gemini、OpenCode 和 DSH 的 reply detection 不应被写成同一套字符串规则。每个 provider 的 session 文件、日志格式、prompt 边界和完成判断都不同。因此 CCB 把 provider-specific 检测放在 provider/completion 层，并通过 manifest 暴露给 dispatcher。DSH 只有在同一 native turn 中同时证明 exact \src{source.rpcId}、非空 committed assistant reply 与 \src{turn/end(completed)} 时才能成功。
