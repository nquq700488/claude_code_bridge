...
当前 tools 命令集中在 Neovim：

\begin{itemize}
  \item \cmd{ccb tools doctor neovim}
  \item \cmd{ccb tools install neovim}
  \item \cmd{ccb tools update neovim}
\end{itemize}

实现位于 \src{lib/cli/tools_runtime/neovim.py}。它使用 XDG data/state/cache roots，在 \src{ccb/tools/neovim} 下维护 managed Neovim/LazyVim profile。环境变量 \src{CCB_INSTALL_NEOVIM}、\src{CCB_LAZYVIM_PROFILE}、\src{CCB_LAZYVIM_SYNC_TIMEOUT_S}、\src{CCB_LAZYVIM_BOOTSTRAP_TIMEOUT_S} 会影响行为。
