...
\begin{itemize}
  ...
  \item 可以用 \cmd{--agent NAME} 指定 agent 名。
...
当前 tools 命令支持 managed Neovim：

\begin{longtable}{p{0.36\linewidth}p{0.54\linewidth}}
\toprule
命令 & 作用 \\
\midrule
\cmd{ccb tools doctor neovim} & 检查 managed Neovim 状态。 \\
\cmd{ccb tools install neovim} & 安装或配置 managed Neovim。 \\
\cmd{ccb tools update neovim} & 更新 managed Neovim/LazyVim profile。 \\
\bottomrule
\end{longtable}
...
\begin{itemize}
  \item \src{CCB_INSTALL_NEOVIM}
  \item \src{CCB_LAZYVIM_PROFILE}
  \item \src{CCB_LAZYVIM_SYNC_TIMEOUT_S}
  \item \src{CCB_LAZYVIM_BOOTSTRAP_TIMEOUT_S}
\end{itemize}

managed Neovim 使用 XDG data/state/cache 目录下的 \src{ccb/tools/neovim} 维护状态。
