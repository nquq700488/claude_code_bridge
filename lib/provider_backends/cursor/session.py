...
def find_project_session_file(work_dir: Path, instance: Optional[str] = None) -> Optional[Path]:
    return _find_project_session_file(work_dir, provider="cursor", session_filename=".cursor-session", instance=instance)


def load_project_session(work_dir: Path, instance: Optional[str] = None):
    return load_native_project_session(work_dir, provider="cursor", session_filename=".cursor-session", instance=instance)


def build_session_binding() -> ProviderSessionBinding:
    return build_native_session_binding(provider="cursor", session_filename=".cursor-session")
