...
def find_project_session_file(work_dir: Path, instance: Optional[str] = None) -> Optional[Path]:
    return _find_project_session_file(work_dir, provider="zai", session_filename=".zai-session", instance=instance)


def load_project_session(work_dir: Path, instance: Optional[str] = None):
    return load_native_project_session(work_dir, provider="zai", session_filename=".zai-session", instance=instance)


def build_session_binding() -> ProviderSessionBinding:
    return build_native_session_binding(provider="zai", session_filename=".zai-session")
