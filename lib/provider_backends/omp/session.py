...
def load_project_session(work_dir: Path, instance: Optional[str] = None):
    return load_native_project_session(work_dir, provider="omp", session_filename=".omp-session", instance=instance)


def build_session_binding() -> ProviderSessionBinding:
    return build_native_session_binding(provider="omp", session_filename=".omp-session")
