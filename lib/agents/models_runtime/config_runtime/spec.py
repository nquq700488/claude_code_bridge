...
class AgentSpec:
    ...
    def __post_init__(self) -> None:
        ...
        self._validate_runtime_shortcut_env(
            ...
            thinking=thinking,
            env=env,
            provider_profile_env=provider_profile.env,
        )
    ...
    def _validate_runtime_shortcut_env(
        ...
        *,
        ...
        thinking: str | None,
        env: dict[str, str],
        provider_profile_env: dict[str, str],
    ) -> None:
        structured_keys: set[str] = set()
        if model is not None:
            structured_keys.update(provider_model_runtime_env_keys(provider))
        if thinking is not None:
            structured_keys.update(provider_thinking_runtime_env_keys(provider))
        if not structured_keys:
            return
        for source, values in (('env', env), ('provider_profile.env', provider_profile_env)):
            overlap = sorted(structured_keys & set(values))
            if overlap:
                raise AgentValidationError(
                    f'model/thinking cannot be combined with {source} runtime overrides: {", ".join(overlap)}'
                )
    ...
    def _normalize_startup_args(
        ...
        *,
        ...
        try:
            compiled_model_args = (
                provider_model_startup_args(provider, model=model) if model is not None else ()
            )
            compiled_thinking_args = provider_thinking_startup_args(provider, thinking=thinking)
        except ValueError as exc:
            raise AgentValidationError(str(exc)) from exc
        compiled_prefix = (*compiled_model_args, *compiled_thinking_args)
        if compiled_prefix and startup_args[: len(compiled_prefix)] == compiled_prefix:
            remaining_args = startup_args[len(compiled_prefix) :]
        else:
            remaining_args = startup_args
            if compiled_model_args and remaining_args[: len(compiled_model_args)] == compiled_model_args:
                remaining_args = remaining_args[len(compiled_model_args) :]
            if compiled_thinking_args and remaining_args[: len(compiled_thinking_args)] == compiled_thinking_args:
                remaining_args = remaining_args[len(compiled_thinking_args) :]
        if model is not None and startup_args_contain_model_flag(provider, remaining_args):
            raise AgentValidationError(
                f'model cannot be combined with startup_args model flags for provider {provider}'
            )
        if thinking is not None and startup_args_contain_thinking_flag(provider, remaining_args):
            raise AgentValidationError(
                f'thinking cannot be combined with startup_args reasoning flags for provider {provider}'
            )
        return (*compiled_prefix, *remaining_args)
