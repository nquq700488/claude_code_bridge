...
from cli.services.team_lifecycle import team_list, team_start, team_status, team_stop, team_ui
...
def _team_lifecycle(context, command):
    ...
    if action == 'start':
        return team_start(context, command)
    if action == 'stop':
        return team_stop(context, command)
    if action == 'status':
