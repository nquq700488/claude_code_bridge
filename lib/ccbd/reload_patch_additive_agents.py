...
def additive_agent_steps(old_topology, new_topology, *, step_factory) -> dict[str, object]:
    ...
    for window_name, new_window in new_windows.items():
        ...
        if old_window is None:
            continue
        result = _steps_for_window(window_name, old_window, new_window, step_factory=step_factory)
        steps.extend(result['steps'])
        blocked.extend(result['blocked'])
    return {'steps': steps, 'blocked': blocked}


def _steps_for_window(window_name: str, old_window, new_window, *, step_factory) -> dict[str, object]:
    ...
    return {'steps': _append_steps(window_name, old_agents, append_plan, step_factory=step_factory), 'blocked': []}


def _append_steps(window_name: str, old_agents: tuple[str, ...], append_plan, *, step_factory) -> list[object]:
