...
3. Request fidelity:
   - `+ --artifact-request`: exact transient input
     ...
     Poke repo paths when the target can read files directly.
...
4. Present the reply to the user. If the reply is empty, tell the user the target agent has not responded yet.
