...
SCHEMA_VERSION = 1
DEFAULT_RESULT_PATH = REPO_ROOT / 'dev_tools' / 'perf_results' / 'python_rust_phase8_job_summary_projection_helper.json'
...
def run_phase8_job_summary_projection_helper(options: Phase8Options) -> dict[str, Any]:
    ...
    try:
        ...
        python_metric = _measure(
            lambda: _summary_signature(store.list_agent_tail_summaries_batch(agent_names, limit=max(0, options.tail))),
        ...
        if helper_bin is not None:
            helper_metric = _measure(
                lambda: _read_with_helper(store, agent_names, limit=max(0, options.tail), helper_bin=helper_bin),
                iterations=max(1, options.iterations),
            )
            python_value = _summary_signature(store.list_agent_tail_summaries_batch(agent_names, limit=max(0, options.tail)))
            helper_value = _read_with_helper(store, agent_names, limit=max(0, options.tail), helper_bin=helper_bin)
        ...
        result = {
            ...
            'phase': 'phase8_job_summary_projection_helper',
            ...
            'parity': parity,
            'metrics': {
                'python_job_summary_projection': python_metric,
                'rust_helper_job_summary_projection': helper_metric,
            },
            'speedup_p50': speedup,
...
def _fixture_root(requested: Path | None) -> tuple[Path, tempfile.TemporaryDirectory[str] | None]:
    if requested is None:
        temp = tempfile.TemporaryDirectory(prefix='ccb-phase8-job-summary-')
            ...
            store.append(
                JobRecord(
                    ...
                    request=MessageEnvelope(
                        ...
                        to_agent=agent_name,
                        from_actor='cmd',
                        body=f'job summary projection {agent_name} {index}',
                        task_id=f'task-{index}' if index % 13 == 0 else None,
                        reply_to='agent0' if index % 17 == 0 else None,
                        message_type='ask',
...
def _read_with_helper(
    ...
    *,
    ...
    with _temporary_env({'CCB_RUST_JOB_SUMMARY_TAIL': '1', RUST_HELPER_BIN_ENV: str(helper_bin)}):
        return _summary_signature(store.list_agent_tail_summaries_batch(agent_names, limit=limit))


def _summary_signature(records_by_agent) -> tuple[tuple[str, tuple[tuple[str, str, str, str], ...]], ...]:
    return tuple(
        (
            str(agent_name),
            tuple(
        ...
        for agent_name, records in sorted(records_by_agent.items())
    )
...
def _measure(call, *, iterations: int) -> dict[str, object]:
    ...
    for _ in range(iterations):
        ...
        if isinstance(value, tuple):
            agent_count = len(value)
            summary_count = sum(len(records) for _, records in value)
    return {
        'status': 'measured',
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Benchmark Phase 8 job summary projection helper behavior.')
    ...
    result = run_phase8_job_summary_projection_helper(
        Phase8Options(
