...
def run_phase6_jsonl_store_strict_helper(options: Phase6Options) -> dict[str, Any]:
    ...
    try:
        ...
        python_metric = _measure(
            lambda: _job_ids(store.list_agent_tails_batch(agent_names, limit=max(0, options.tail))),
        ...
        if helper_bin is not None:
            ...
            python_value = _job_ids(store.list_agent_tails_batch(agent_names, limit=max(0, options.tail)))
        ...
        result = {
            ...
            'phase': 'phase6_jsonl_store_strict_helper',
            ...
            'parity': parity,
            'metrics': {
                'python_jobstore_batch_tail': python_metric,
                'rust_helper_jobstore_batch_tail': helper_metric,
            },
            'speedup_p50': speedup,
        ...
        for index in range(rows_per_agent):
            store.append(
                JobRecord(
                    ...
                    updated_at=f'2026-06-15T00:{index % 60:02d}:{index % 60:02d}Z',
                )
            )
...
def _job_ids(records_by_agent: dict[str, list[JobRecord]]) -> dict[str, list[str]]:
    return {
        agent_name: [record.job_id for record in records]
...
def _measure(call, *, iterations: int) -> dict[str, object]:
    ...
    for _ in range(iterations):
        ...
        if isinstance(value, dict):
            ...
            job_count = sum(len(rows) for rows in value.values())
    return {
        'status': 'measured',
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Benchmark Phase 6 strict JSONL JobStore helper behavior.')
    ...
    result = run_phase6_jsonl_store_strict_helper(
        Phase6Options(
