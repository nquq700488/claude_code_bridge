...
SCHEMA_VERSION = 1
DEFAULT_RESULT_PATH = REPO_ROOT / 'dev_tools' / 'perf_results' / 'python_rust_phase4_storage_scan_helper.json'
...
def run_phase4_storage_scan_helper(options: Phase4Options) -> dict[str, Any]:
    ...
    try:
        ...
        python_metric = _measure(
            lambda: _summarize_with_env(layout, storage_mode='0', helper_bin=None),
        ...
        if helper_bin is not None:
            helper_metric = _measure(
                lambda: _summarize_with_env(layout, storage_mode='1', helper_bin=helper_bin),
                iterations=max(1, options.iterations),
            )
            python_payload = _summarize_with_env(layout, storage_mode='0', helper_bin=None)
            helper_payload = _summarize_with_env(layout, storage_mode='1', helper_bin=helper_bin)
            parity = _parity_result(python_payload, helper_payload)
        ...
        result = {
            ...
            'phase': 'phase4_storage_scan_helper',
            ...
            'parity': parity,
            'metrics': {
                'python_storage_summary': python_metric,
                'rust_helper_storage_summary': helper_metric,
            },
            'speedup_p50': speedup,
            'integration_gate': {
...
def _fixture_root(requested: Path | None) -> tuple[Path, tempfile.TemporaryDirectory[str] | None]:
    if requested is None:
        temp = tempfile.TemporaryDirectory(prefix='ccb-phase4-storage-')
...
def _summarize_with_env(layout: PathLayout, *, storage_mode: str, helper_bin: Path | None) -> dict[str, object]:
    updates = {RUST_STORAGE_SCAN_ENV: storage_mode}
    if helper_bin is not None:
        updates[RUST_HELPER_BIN_ENV] = str(helper_bin)
    with _temporary_env(updates):
        return summarize_storage(layout)
...
def _parity_result(python_payload: dict[str, object], helper_payload: dict[str, object]) -> dict[str, object]:
    if _stable_summary(python_payload) == _stable_summary(helper_payload):
        return {'matches_python': True, 'reason': 'matched'}
    return {
        'matches_python': False,
        'reason': 'summary_mismatch',
        'python_total_count': python_payload.get('total_count'),
        'helper_total_count': helper_payload.get('total_count'),
        'python_total_bytes': python_payload.get('total_bytes'),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Benchmark Phase 4 storage scan helper behavior.')
    ...
    result = run_phase4_storage_scan_helper(
        Phase4Options(
